...
def get_msp_and_prediction(model_pre, tokenizer_pre, x_s, x_a):
    ...
    # # 验证当前模式
    # current_mode = ms.context.get_context("mode")
    ...
    # print(f"当前模式: {current_mode}")
    # # 确认模式是否切换成功
    # if current_mode != ms.PYNATIVE_MODE:
    #     raise RuntimeError("无法切换到动态图模式")
    # 去除梯度信息
    # logits = ops.stop_gradient(logits)
    # logits=ms.Tensor(logits)
    # 确保 logits 是一个 Tensor
    # print(logits)
    # print(logits.data)
    logits_data=logits.data
    # if not isinstance(logits_data, ms.Tensor):
    #     logits_tensor=ms.Tensor(logits_data)
    probs = manual_softmax(logits_data)

    # logits_data_array=logits.asnumpy()
    logits_data_list=logits_data.tolist()
    # 获取前 K 个最大值及其索引，这里 k=1
    # values, pred_label = ops.topk(logits,k=1)
    # pred_label = ops.argmax(logits_data_list)
    # 使用enumerate函数获取每个元素及其索引
    # max函数的key参数指定比较的依据，这里是比较元素的值
